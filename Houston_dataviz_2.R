...
HospitalPrices <-read.csv("HospitalPrices.csv", as.is=T)
...
code = drg[48]
hos3 <-subset(HospitalPrices2, DRG==code)
hos3 <-subset(hos3, State=='TX')
TX <- qmap("texas", zoom=6, color="bw", legend = "none")
TX +  geom_point(data=hos3, aes(x=lon, y=lat, color=AveragedCoveredCharges, alpha=0.6, size=Freq*10)) +
  scale_colour_continuous(low="red", high="black")+ggtitle(paste("DRG code:", code, sep=""))
